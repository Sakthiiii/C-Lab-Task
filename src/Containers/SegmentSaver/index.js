...
const SegmentSaver = (props) => {
  ...
  const availableOptions =
    options?.filter((eOption) => {
      const foundArr =
        schemas?.find((eSchema) => eSchema.value === eOption.value) ?? {};
  ...
  const onChangeEachSchema = (e, prev) => {
    const updatedSchema = [...schemas].filter(
      (eSchema) => eSchema.value !== prev
    );
    const currentSchema = availableOptions?.find(
      (eOption) => eOption.value === (e ? e.currentTarget.value : "")
    );
    setSchemas([...updatedSchema, currentSchema]);
  };
  ...
  const onClickAddNewSchema = () => {
    const newSchema = availableOptions?.find(
      (eOption) => eOption.value === activeSchema ?? ""
    );
  ...
  return (
    <React.Fragment>
      <div className="cl-segment-saver-container">
        ...
        {schemas?.length > 0 && (
          <div className="cl-selected-schemas">
            {schemas?.map(({ value, label }) => (
              <div className="cl-select-del-marker-wrap" key={`wrap_${value}`}>
                <div className="cl-circle-icon-container">
        ...
        <div
          className="cl-select-del-marker-wrap"
          key={`wrap_add_new_segment}`}
        >
          <div className="cl-circle-icon-container">
            <FontAwesomeIcon icon={faCircle} color="#e6e8eb" />
          </div>
        ...
        <div className="cl-add-new-schema-container">
          <AnchorButton
            disabled={!activeSchema}
            onClick={onClickAddNewSchema}
            icon={"+"}
          >
            Add new schema
          </AnchorButton>
